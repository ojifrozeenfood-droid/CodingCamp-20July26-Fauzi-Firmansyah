...
- [x] 2. Implement the DataStore module
  ...
  - [x] 2.3 Write property test for DataStore round-trip (Property 2)
  ...
  - [x] 2.5 Write property test for budget upsert idempotence (Property 4)
  ...
  - [x] 2.6 Write property test for delete completeness (Property 5)
  ...
  - [x] 2.7 Write property test for unique IDs (Property 9)
...
- [x] 3. Implement Validator module
  ...
  - [x] 3.3 Write property test for validation rejection of invalid inputs (Property 6)
...
- [x] 5. Implement aggregation and filter logic
  ...
  - [x] 5.3 Write property test for aggregation consistency (Property 1)
  ...
  - [x] 5.5 Write property test for month filter exclusivity (Property 3)
  ...
  - [x] 5.7 Write property test for six-month window (Property 10)
  ...
  - [x] 5.9 Write property test for budget percentage invariant (Property 7)
...
- [x] 6. Implement ChartRenderer — Canvas utility helpers
  - [x] 6.1 Implement `resizeCanvas(canvas)`
  ...
  - [x] 6.2 Implement `drawEmptyState(ctx, canvas, message)`
    - Center the message text on the canvas using `ctx.fillText` with appropriate font and color
    - _Requirements: 6.6_

  - [x] 6.3 Implement `drawYAxis(ctx, padding, chartHeight, maxValue)` and `drawXAxisLabels(ctx, padding, chartWidth, chartHeight, months)`
    - Draw Y-axis line, tick marks at even intervals, value labels; draw X-axis labels for month abbreviations
    - _Requirements: 7.5_

- [x] 7. Implement ChartRenderer — Pie chart
  - [x] 7.1 Implement `renderPieChart(canvas, monthSummary)`
  ...
  - [x] 7.2 Implement `drawPieLegend(ctx, canvas, monthSummary, totalExpense)`
    - Render colored square + category name + percentage for each category with non-zero spend, below the pie circle
    - _Requirements: 6.5_

- [x] 8. Implement ChartRenderer — Bar chart
  - [x] 8.1 Implement `renderBarChart(canvas, store, activeMonthKey)`
  ...
  - [x] 8.2 Implement `drawBarLegend(ctx, canvas, padding)`
    - Render small colored rectangles + labels for "Anggaran" (budget) and "Aktual" (actual) at top-right of chart area
    - _Requirements: 7.6_

- [x] 9. Implement UIController — Progress bars
  - [x] 9.1 Implement `renderProgressBars(container, comparisons)`
  ...
  - [x] 9.2 Write property test for progress bar color thresholds (Property 11)
...
- [x] 10. Implement UIController — Transaction list and form
  - [x] 10.1 Implement `renderTransactionList(container, transactions, monthKey)`
  ...
  - [x] 10.5 Implement delete transaction handler (event delegation on transaction list)
...
- [x] 11. Implement UIController — Month filter and budget panel
  - [x] 11.1 Implement `populateMonthFilter(selectElement, transactions)`
  ...
  - [x] 11.2 Implement month filter change handler
    - On select change, update `FilterState.activeMonth`; call `renderTransactionList`; call `ChartRenderer.renderAll`
    - _Requirements: 4.3, 4.4_

  - [x] 11.3 Implement `renderBudgetPanel(container, budgets, activeMonth)`
...
- [ ] 13. Implement responsive CSS
  - [x] 13.1 Write mobile-first CSS layout rules
...
- [ ] 14. Implement app initialization and final wiring
  ...
  - [x] 14.2 Implement `ChartRenderer.renderAll(activeMonth)`
  ...
  - [x] 14.3 Wire `DOMContentLoaded` event to `initApp()`
